...
# ceate constant vars for validation of connected APIs
...
def get_project_type():
    while True:
        ...
        if new_type.strip().lower() == "k":
            return "Kitchen", "2"
        elif new_type.strip().lower() == "b":
            return "Beverages", "6"
        elif new_type.strip().lower() == "d":
...
def load_project(user_email):
    ...
    print(
        ...
        + f"{space}{title}"
        + Style.RESET_ALL
        + f" * {user_email} Welcome to your project Page * \n"
    )
    while True:
...
def calculate_capacity(new_type, new_insulation, new_volume, new_floor):
    ...
    if (temp_range, new_insulation) not in base_and_steps:
        print(
            f"{space}Unknown temp range and insulation combination: {temp_range}, {new_insulation}"  # noqa
        )
        return
...
def insulation_thickness():
    ...
    while True:
        try:
            ...
            if insulation.strip().lower() == "a":
                return "70mm"
            elif insulation.strip().lower() == "b":
                return "100mm"
            else:
                print(f"{space}Please enter a valid option\n")
        except ValueError as e:
...
def check_email_registered():
    ...
    while True:
        ...
        print(
            ...
            title
            + Style.RESET_ALL
            + f"{space}* Welcome to the Coldroom Calculator * \n",
        )
        ...
        if user_email in mail:
            ...
            break
        elif user_email.strip().lower() == "i":
            print(f"{space}Loading info page...... \n")
            instruction_page()
            break
        elif user_email.strip().lower() == "q":
...
def register_user(mail):
    ...
    while True:
        new_user_email = input(
            f"{space}Please enter your email address to register: \n {space}or enter [b] to go back.\n{space}"  # noqa
        )
        try:
            valid_new_user_email = validate_email(
                new_user_email, check_deliverability=False
            )
        ...
        if new_user_email.strip().lower() == "b":
            clear()
...
def create_new_user_name(new_email):
    while True:
        ...
        if len(new_user_name) <= 8 and len(new_user_name) > 0:
            ...
            print(
                f"{space}{new_user_name} welcome to the Coldroom Capacity Calculator. "  # noqa
            )
